class Solution:
    ...
    def minct(self, nums, memo):
        if (nums[0],len(nums)) in memo:
            return memo[(nums[0],len(nums))]
        if len(nums)<3:
            return max(nums)
        else:
            temp = nums[:3]
        ...
        nums3 = [temp[1]]+nums
        print(temp[1],temp[2])
        p1, p2,p3 = self.minct(nums1, memo)+temp[1], self.minct(nums2,memo)+temp[2], self.minct(nums3, memo)+temp[2]

        memo[(nums1[0],len(nums1))] = p1
        memo[(nums2[0],len(nums2))] = p2
        memo[(nums3[0],len(nums3))] = p3
        print(temp[1],'-', p1, temp[2], '-',  p2, p3)
        return min(p1, p2, p3)
